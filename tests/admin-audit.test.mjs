...
test("6c: 052 is the newest migration, additive, and edits none of the 51 before it", () => {
  const files = readdirSync(path.join(ROOT, "supabase/migrations")).filter(f => f.endsWith(".sql")).sort();
  assert.equal(files.length, 52);
  assert.equal(files[files.length - 1], "052_admin_activity_audit.sql",
    "052 is not the newest migration");
