...
test("38-39: 022 through 035 are all present and there is no 036", () => {
  ...
  assert.equal(files.length, 61, "a migration was added or removed");
  ...
  assert.equal(files[files.length - 27], MIGRATION_035);
  assert.deepEqual(files.filter(f => f.startsWith("037")), ["037_subscription_refund_correlation.sql"]);
  assert.ok(!files.some(f => f.startsWith("062")), "an unreviewed migration appeared");
