...
test("1: exactly one 039 exists and it is the highest migration", () => {
  ...
  assert.equal(files[files.length - 16], MIGRATION_039, "039 must be the highest");
  assert.equal(files[files.length - 17], MIGRATION_038, "038 must be the one before it");
...
test("2: no migration 044 or beyond", () => {
  ...
  const beyond = readdirSync(MIGRATIONS_DIR).filter(f => Number(f.slice(0, 3)) > 54);
...
test("54: no UNCOMMITTED edit to a live application module is in the working tree", () => {
  ...
  const ALLOWED_LIB_EDITS = [
    ...
    //
    // 1. A withheld product (lib/catalogAvailability.ts) is refused with
    //    the status and the sentence an inactive product already got.
    //    The GLOA Metal Case is active in the catalog and must not be
    //    sold; hiding its card never stopped this endpoint pricing it.
    // 2. An exhausted catalog read reports 503 instead of 500. Same
    //    customer-facing sentence; only the class of the answer changes,
    //    from "this request was bad" to "this is temporary".
    //
    // Nothing else moved: the item validation, the price/size/currency
    // checks, the active-product checks, the arithmetic and the
    // annual-plan and subscription paths are byte-identical, and no
    // retry, clock or write was introduced. Reviewed directly against
