...
test("1: 038 exists, owns its number, and 039 is the only one above it", () => {
  ...
  assert.equal(files[files.length - 8], MIGRATION_039, "039 must be the highest");
  assert.equal(files[files.length - 9], MIGRATION_038, "038 must be the one before it");
  assert.equal(files[files.length - 10], MIGRATION_037, "037 must be the one before that");
...
test("2: no migration 044 or beyond", () => {
  ...
  assert.deepEqual(beyond, [MIGRATION_039, MIGRATION_040, MIGRATION_041, MIGRATION_042,
    "043_launch_waitlist.sql", "044_launch_send.sql", "045_launch_welcome_email.sql", "046_launch_signup_atomic.sql"],
