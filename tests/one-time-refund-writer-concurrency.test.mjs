...
test("1: 038 exists, owns its number, and 039 is the only one above it", () => {
  ...
  assert.equal(files[files.length - 7], MIGRATION_039, "039 must be the highest");
  assert.equal(files[files.length - 8], MIGRATION_038, "038 must be the one before it");
  assert.equal(files[files.length - 9], MIGRATION_037, "037 must be the one before that");
