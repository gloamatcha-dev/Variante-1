...
test("1: 038 exists, owns its number, and 039 is the only one above it", () => {
  ...
  assert.equal(files[files.length - 18], MIGRATION_037, "037 must be the one before that");
...
const changedFiles = () => {
  const changed = execFileSync("git", ["diff", "--name-only", "--diff-filter=MD", "HEAD"], { cwd: ROOT, encoding: "utf-8" }).trim();
