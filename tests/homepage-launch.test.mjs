...
test("39: every homepage section starts on one rail, and it is the lifestyle one", () => {
  ...
  assert.match(railCss, /\.countdown,\s*\.prelaunch,\s*\.daily,\s*\.origin,\s*\.how-to,\s*\.community,\s*\.brand-note,\s*\.shop-hero,\s*\.shop-strip,\s*\.shop-column,\s*\.shop-accordion\{padding-inline:var\(--rail-gutter\)\}/);
