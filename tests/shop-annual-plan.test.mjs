...
test("5d: no backend, migration or commercial logic changed", () => {
  ...
  assert.ok(!readdirSync(path.join(ROOT, "supabase/migrations")).some(f => f.startsWith("046")),
    "migration 044 exists");
  assert.equal(readdirSync(path.join(ROOT, "supabase/migrations")).filter(f => f.endsWith(".sql")).length, 45,
