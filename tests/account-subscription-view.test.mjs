...
test("3F: only the owner's own private subscriptions are readable", () => {
  ...
  assert.equal(migrations.length, 54, "an unreviewed migration was added");
  assert.deepEqual(
...
test("3F: migrations 022 through 034 are untouched and only 035, 036 and 037 follow", () => {
  ...
  // here.
  assert.equal(files[files.length - 18], "037_subscription_refund_correlation.sql");
  assert.equal(files[files.length - 17], "038_one_time_refund_writer_concurrency.sql");
  assert.equal(files[files.length - 16], "039_b2c_annual_plan_foundation.sql");
