...
test("3F: only the owner's own private subscriptions are readable", () => {
  ...
  assert.equal(migrations.length, 59, "an unreviewed migration was added");
  assert.deepEqual(
    ...
    ["035_subscription_email_deliveries.sql", "036_subscription_payment_status.sql",
     ...
     // Phase 4B8.1. Column-level privileges on the two ANNUAL tables. It
     ...
     // 044: the one-time launch send - claim columns on launch_waitlist,
     // a release flag and three functions. 045: the welcome mail with
     // the discount code - watermark and claim columns on the same
     // table. Both are additive, both are confined to the launch
     // feature, and neither touches a subscription table, policy or
     // grant. Reviewed in tests/launch-send.test.mjs and
     // tests/launch-waitlist.test.mjs.
     // 046: the atomic signup and the split between the consent in force
     // and a merely proposed one. Confined to launch_waitlist, touches no
     // subscription table, policy or grant.
  ...
  // 039 is Phase 4B1's B2C prepaid annual plan foundation, reviewed in
  // tests/annual-plan-foundation-migration.test.mjs. It is the first
  // migration above 034 that DOES create policies and DOES grant to
  // authenticated, so the property is restated precisely rather than
  // waived: everything it touches, it created itself. It adds no policy
...
test("3F: migrations 022 through 034 are untouched and only 035, 036 and 037 follow", () => {
  ...
  // here.
  assert.equal(files[files.length - 23], "037_subscription_refund_correlation.sql");
  assert.equal(files[files.length - 22], "038_one_time_refund_writer_concurrency.sql");
  assert.equal(files[files.length - 21], "039_b2c_annual_plan_foundation.sql");
  assert.equal(files[files.length - 20], "040_annual_checkout_retry_fingerprints.sql");
  assert.equal(files[files.length - 19], "041_annual_account_column_privileges.sql");
  ...
  assert.equal(files[files.length - 14], "046_launch_signup_atomic.sql");
  assert.equal(files[files.length - 15], "045_launch_welcome_email.sql");
  assert.equal(files[files.length - 16], "044_launch_send.sql");
  assert.equal(files[files.length - 17], "043_launch_waitlist.sql");
  assert.equal(files[files.length - 18], "042_annual_delivery_rls_parent_user_privilege.sql");
