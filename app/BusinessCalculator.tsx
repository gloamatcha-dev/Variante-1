...
// Shown when the enquiry could not be delivered. It says what happened
// and what to do, and it never shows a status code or a provider
// message - the visitor cannot act on either.
const SEND_ERROR = "Die Anfrage konnte gerade nicht gesendet werden. Bitte versuche es noch einmal.";
...
export function BusinessCalculator(){
 ...
 }),[success,setSuccess]=useState(false),[sending,setSending]=useState(false),[sendError,setSendError]=useState("");
 ...
 const submit=async(e:React.FormEvent<HTMLFormElement>)=>{e.preventDefault();if(sending)return;const f=new FormData(e.currentTarget);const payload:LeadPayload={lead_type:intent,contact_name:String(f.get("contact_name")),business_name:String(f.get("business_name")),email:String(f.get("email")),city:String(f.get("city")),business_type:String(f.get("business_type")),locations:String(f.get("locations")),pricing_interest:String(f.get("pricing_interest")||""),estimated_monthly_demand:String(f.get("demand")||""),current_supplier:String(f.get("supplier")||""),message:String(f.get("message")||""),created_at:new Date().toISOString()};setSending(true);setSendError("");try{const res=await fetch("/api/b2b-lead",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});if(!res.ok){setSendError(SEND_ERROR);return}}catch{setSendError(SEND_ERROR);return}finally{setSending(false)}window.dispatchEvent(new CustomEvent("gloa:b2b-lead",{detail:payload}));track(intent==="sample"?"sample_request_submit":"wholesale_request_submit");setSuccess(true)};
...
<section className="sample-callout"><div className="sample-callout-inner home-rail"><div className="sample-callout-copy"><p className="eyebrow sample-callout-eyebrow">PROBIER&apos;S ERST MAL.</p><h2 className="sample-callout-headline"><span className="sample-callout-line">Test it with</span><span className="sample-callout-line">your team.</span></h2><span className="sample-callout-rule" aria-hidden="true"/></div><div className="sample-callout-detail"><p className="sample-callout-body">Bestell ein Sample, bevor du deine erste größere Bestellung aufgibst. Konditionen klären wir individuell.</p><button className="cta sample-callout-cta" onClick={()=>choose("sample")}>Sample anfragen</button></div></div></section>

<section id="lead" className="lead-section">{success?<div className="form-success home-rail"><p className="eyebrow lead-form-eyebrow">GLOA FOR BUSINESS</p><h2>{intent==="sample"?"Deine Sample-Anfrage ist drin.":"Danke. Wir melden uns."}</h2><p>Wir haben deine Anfrage erhalten und melden uns mit den nächsten Schritten.</p></div>:<form className="lead-form home-rail" onSubmit={submit}><p className="eyebrow wide lead-form-eyebrow">GLOA FOR BUSINESS</p><h2 className="lead-form-headline"><span className="lead-form-line">Let&apos;s talk</span><i className="lead-form-line lead-form-line-accent">Matcha.</i></h2><div className="intent-tabs"><button type="button" className={intent==="wholesale"?"active":""} onClick={()=>setIntent("wholesale")}>B2B-Anfrage</button><button type="button" className={intent==="sample"?"active":""} onClick={()=>setIntent("sample")}>Sample</button></div><label>Ansprechperson*<input required name="contact_name"/></label><label>Unternehmen / Café*<input required name="business_name"/></label><label>E-Mail*<input required type="email" name="email"/></label><label>Stadt*<input required name="city"/></label><label>Unternehmenstyp*<select required name="business_type" defaultValue=""><option value="" disabled>Bitte auswählen</option>{["Eigenständiges Café","Café-Gruppe","Restaurant","Hotel","Büro / Office","Fitness / Pilates / Wellness","Einzelhandel","Sonstiges"].map(x=><option key={x}>{x}</option>)}</select></label><label>Anzahl Standorte*<input required min="1" type="number" name="locations"/></label><label>Interesse an<select name="pricing_interest"><option>Noch nicht sicher</option><option>Einzelbestellung</option><option>Regelmäßige Belieferung · 5 %</option><option>12-Monats-Partnerschaft · 10 %</option><option>Sample</option></select></label><label>Geplanter monatlicher Bedarf<select name="demand"><option>Noch nicht sicher</option><option>Unter 1 kg</option><option>1-2 kg</option><option>3-5 kg</option><option>6-10 kg</option><option>10+ kg</option></select></label><label>Aktueller Matcha-Lieferant<input name="supplier"/></label><label className="wide">Nachricht<textarea name="message"/></label><label className="consent wide"><input required type="checkbox"/> Ich stimme zu, dass GLOA meine Angaben zur Bearbeitung meiner Anfrage verwenden darf.</label>{sendError&&<p className="lead-form-error" role="alert">{sendError}</p>}<button className="cta cream wide" disabled={sending}>{sending?"WIRD GESENDET…":intent==="sample"?"Sample-Anfrage senden":"B2B-Konditionen anfragen"}</button></form>}</section></>
}
