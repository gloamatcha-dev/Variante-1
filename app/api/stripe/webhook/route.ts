...
import {
  ...
  settleB2bCheckoutSession,
  settleB2bPaidInvoice,
  subscriptionIdOf,
} from "../../../../lib/b2bWebhook";
// Package 5D. An ANNUAL instalment invoice is issued by GLOA and carries
// its own metadata; a monthly one is generated by Stripe from a
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    if (event.type === "checkout.session.completed") {
      ...
      // entirely unchanged.
      // Package 5D FIRST: an annual instalment invoice is recognised by
      // its OWN metadata, which only GLOA writes. It must be tried
      // before the monthly path, which re-reads a SUBSCRIPTION an annual
      // invoice does not have.
      const b2bAnnual = await handleB2bAnnualInvoicePaid(stripe, event);
      if (!b2bAnnual.handled) {
      ...
      // Package 5F replaced the temporary no-op this branch carried in
      // 5C. See handleB2bInvoiceFailed below.
 ...
 */
/**
 * The ANNUAL half of invoice.paid (Package 5D).
 *
 * Returns handled:false for every invoice that is not a GLOA-issued
 * annual instalment, which keeps both the B2B monthly path and the
 * consumer path exactly as they were.
 *
 * It creates NO delivery. An annual agreement received all twelve slots
 * when it activated; a later instalment pays for deliveries that already
 * exist. A success also releases this package's own holds - and migration
 * 063 refuses to release while any other instalment is still owed.
 */
async function handleB2bAnnualInvoicePaid(
  stripe: Stripe,
  event: Stripe.Event
  ...
  const outcome = await settleB2bAnnualPaidInvoice(
    eventInvoice.id, b2bWebhookDeps(stripe), routeB2bAnnualInvoice
  );

  if (outcome.kind === "not_annual") return { handled: false };
  if (outcome.kind === "refused") {
    throw new Error(
      `b2b annual invoice ${eventInvoice.id} could not be settled: ${outcome.reason}`
    );
  }
  console.error(
    `Stripe webhook: b2b annual invoice ${eventInvoice.id} -> ${outcome.kind} `
...
/**
 * The B2B half of invoice.payment_failed (Package 5F).
 *
 * ── THIS REPLACED THE TEMPORARY NO-OP FROM PACKAGE 5C ─────────
 *
 * 5C classified a B2B failure and then did nothing, because no hold
 * state machine existed. It does now, and the rule is the approved one:
 * A PAYMENT FAILURE HOLDS DELIVERIES AND LEAVES THE CONTRACT STANDING.
 * Nothing here cancels, terminates or ends an agreement, and nothing
 * here retries a payment - Stripe dunning owns that.
 *
 * TWO POPULATIONS, IN THE ORDER THEY MUST BE TRIED:
 *
 *   ANNUAL   the invoice carries GLOA's own metadata. The exact
 *            instalment moves to payment_failed and unresolved
 *            deliveries are held.
 *   MONTHLY  the invoice is Stripe's, and the agreement is found through
 *            the SUBSCRIPTION's metadata. There is no payment row to
 *            write - 060 forbids one on a monthly agreement - so the
 *            failure is expressed entirely as the hold.
 *
 * A consumer invoice matches neither and reaches the existing handler
 * untouched.
 */
async function handleB2bInvoiceFailed(
  ...
  // ── ANNUAL FIRST, on the invoice's own metadata ──────────────
  const annual = await recordB2bAnnualInvoiceFailure(
    eventInvoice.id, b2bWebhookDeps(stripe), routeB2bAnnualInvoice
  );
  if (annual.kind === "refused") {
    throw new Error(
      `b2b annual invoice ${eventInvoice.id} failure could not be recorded: ${annual.reason}`
    );
  }
  if (annual.kind !== "not_annual") {
    console.error(
      `Stripe webhook: b2b annual invoice ${eventInvoice.id} -> ${annual.kind} `
      + `(agreement ${annual.agreementId}); deliveries held, contract standing.`
    );
    return { handled: true };
  }

  // ── MONTHLY, through the subscription's metadata ─────────────
  ...
  if (held.kind === "refused") {
    throw new Error(
      `b2b monthly agreement ${routed.agreementId} could not be held: ${held.reason}`
    );
  }
  console.error(
    `Stripe webhook: b2b monthly invoice ${eventInvoice.id} failed - `
    + `${held.held} delivery slot(s) held for agreement ${routed.agreementId}; `
    + `contract standing, Stripe dunning continues.`
  );
  return { handled: true };
}
