...
/* WITHHELD FOR THIS LAUNCH, NOT REMOVED. Home() renders this behind
   RECIPES_VISIBLE (app/content.ts). The component, the marquee, the
   cards, the images and the CSS are untouched and it comes back whole
   the moment that flag is true. See the flag's own note for the full
...
<dl><div><dt>LEBENSMITTELBEZEICHNUNG</dt><dd>Matcha (Grünteepulver)</dd></div><div><dt>HERKUNFT</dt><dd>Shizuoka, Japan</dd></div><div><dt>ZUTAT</dt><dd>100 % Matcha-Grünteepulver</dd></div><div><dt>LAGERUNG</dt><dd>{PRODUCT.storage}</dd></div><div><dt>GRÖSSEN</dt><dd>{product.variants.map(x=>x.label).join(" · ")}</dd></div></dl>
