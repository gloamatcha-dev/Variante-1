...
<div className="stack-pair"><section className="pdp-facts"><div><p className="eyebrow">WHAT WE KNOW</p><h2>Clear facts.<br/>Nothing invented.</h2></div><dl><div><dt>HERKUNFT</dt><dd>Shizuoka, Japan</dd></div><div><dt>VERWENDUNG</dt><dd>Latte · Iced · Pur</dd></div><div><dt>LAGERUNG</dt><dd>{PRODUCT.storage}</dd></div><div><dt>GROESSEN</dt><dd>{MATCHA_VARIANTS.map(v=>v.size).join(" · ")}</dd></div></dl></section>
<HowTo/></div>
</main>}
...
<div className="stack-pair"><section className="matcha-image"><img src="/img/gloa-iced.png" alt="Iced Matcha von GLOA"/></section>
<section className="matcha-cta"><h2>Bereit?</h2><div className="matcha-cta-actions"><a className="cta" href="/shop">Zum Shop</a><a className="cta secondary" href="/for-cafes">B2B →</a></div></section></div>
</main>}
...
<section className="contact-hero"><p className="eyebrow">KONTAKT</p><h1>Schreib<br/><i>uns.</i></h1><p className="contact-sub">Waehl den richtigen Weg und deine Nachricht landet da, wo sie hingehoert.</p></section>
<section className="contact-choices"><a href="#customer-form"><span>PRIVATKUNDE</span><h2>Fragen zu<br/>Produkt & Bestellung.</h2><b>Kontaktformular →</b></a><a href="/for-cafes"><span>BUSINESS</span><h2>Grosshandel,<br/>Samples & Cafés.</h2><b>Zum B2B-Bereich →</b></a></section>
<form id="customer-form" className="customer-form" onSubmit={e=>e.preventDefault()}><p className="eyebrow">NACHRICHT SENDEN</p><div className="customer-form-row"><label>Name*<input required placeholder="Vor- und Nachname"/></label><label>E-Mail*<input required type="email" placeholder="deine@email.de"/></label></div><div className="customer-form-row"><label>Anliegen*<select required defaultValue=""><option value="" disabled>Bitte auswaehlen</option><option>Bestellung</option><option>Produkt</option><option>Abo</option><option>Sonstiges</option></select></label><label>Bestellnummer<input placeholder="Optional"/></label></div><label>Nachricht*<textarea required placeholder="Wie koennen wir helfen?" rows={5}/></label><button className="cta" type="submit">Nachricht senden</button></form>
</main>}
